...
export class ImportUtils {
  static async importFromFile<T>(file: File, columns: ImportColumn[]): Promise<ImportResult<T>> {
    try {
  ...
  private static processImportData<T>(rawData: any[][], columns: ImportColumn[]): ImportResult<T> {
    ...
    if (rawData.length === 0) {
      return {
        success: false,
        data: [],
        errors: [{ row: 0, message: "파일이 비어있습니다." }],
        summary: { total: 0, success: 0, failed: 0 },
      }
    }
    ...
    for (let i = 1; i < rawData.length; i++) {
      ...
      const { data: rowData, errors: rowErrors } = this.processRow<T>(row, columns, rowNumber)
  ...
  private static validateHeaders(headers: any[], columns: ImportColumn[]): ImportError[] {
    const errors: ImportError[] = []
  ...
  private static validateField(value: any, column: ImportColumn, rowNumber: number): ImportError[] {
    const errors: ImportError[] = []
    ...
    if (column.required && this.isEmpty(value)) {
      errors.push({
        row: rowNumber,
        field: column.key,
        message: `${column.title}은(는) 필수입니다.`,
        value,
      })
      return errors
    }
    ...
    switch (column.type) {
      case "number":
        if (isNaN(Number(value))) {
          errors.push({
            row: rowNumber,
            field: column.key,
            message: `${column.title}은(는) 숫자여야 합니다.`,
            value,
          })
        }
        break
      ...
      case "email":
        ...
        if (!emailRegex.test(String(value))) {
          errors.push({
            row: rowNumber,
            field: column.key,
            message: `${column.title}의 이메일 형식이 올바르지 않습니다.`,
            value,
          })
        }
        break
      ...
      case "date":
        ...
        if (isNaN(date.getTime())) {
          errors.push({
            row: rowNumber,
            field: column.key,
            message: `${column.title}의 날짜 형식이 올바르지 않습니다.`,
            value,
          })
        }
        break
      ...
      case "boolean":
        ...
        if (!["y", "n", "yes", "no", "true", "false", "1", "0"].includes(boolValue)) {
          errors.push({
            row: rowNumber,
            field: column.key,
            message: `${column.title}은(는) Y/N 또는 True/False 값이어야 합니다.`,
            value,
          })
        }
        break
