...
export interface FormField {
  ...
  group?: string // 필드 그룹
  customRender?: (field: FormField, value: any, onChange: (value: any) => void, error?: string) => React.ReactNode
}
...
export function StandardForm({
  ...
  useEffect(() => {
    ...
    const defaultCollapsed = new Set(groups.filter((group) => group.defaultCollapsed).map((group) => group.name))
    ...
    (field: FormField, value: any): string | null => {
      ...
      if (field.validation) {
        ...
        if (field.type === "number" && value !== "") {
          ...
          if (min !== undefined && numValue < min) return `${field.label}은(는) ${min} 이상이어야 합니다.`
          if (max !== undefined && numValue > max) return `${field.label}은(는) ${max} 이하여야 합니다.`
        }
        ...
        if (typeof value === "string") {
          ...
          if (pattern && !pattern.test(value)) return `${field.label} 형식이 올바르지 않습니다.`
        }
  ...
  const renderField = (field: FormField) => {
    ...
    if (field.customRender) {
      return field.customRender(field, value, (newValue) => handleInputChange(field.name, newValue), error)
    }
        ...
        case "password":
          return (
            <div className="relative">
              <Input type={showPasswords.has(field.name) ? "text" : "password"} {...commonProps} />
              <Button
                type="button"
                variant="ghost"
                size="icon"
                ...
                aria-label={showPasswords.has(field.name) ? "비밀번호 숨기기" : "비밀번호 보기"}
              >
                {showPasswords.has(field.name) ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </Button>
            </div>
          )
        case "number":
        ...
        case "date":
          return (
            <DatePicker
              ...
              onChange={(date) => handleInputChange(field.name, date ? date.toISOString().split("T")[0] : undefined)} // Store as YYYY-MM-DD
              disabled={isDisabled}
              placeholder={field.placeholder}
            />
          )
    ...
    return (
      <div key={field.name} className={`space-y-1.5 ${field.gridColumn || ""}`}>
        {field.type !== "checkbox" && field.type !== "switch" && (
          <Label htmlFor={field.name} className="flex items-center gap-1 text-sm font-medium">
            {field.label}
  ...
  const renderFieldsByGroup = (groupName?: string) => {
    const groupFields = fields.filter((field) => (groupName ? field.group === groupName : !field.group))
    if (groupFields.length === 0 && groupName) return null // Don't render card if no fields for this group

    const content = (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-5">{groupFields.map(renderField)}</div>
    )
    ...
    return (
      <Card key={groupName}>
        <CardHeader>
          ...
          {groupInfo?.description && <CardDescription>{groupInfo.description}</CardDescription>}
        </CardHeader>
  ...
  const allGroups = [
    ...groups,
    ...(fields.some((f) => !f.group) ? [{ name: "__ungrouped__", title: "기타 정보" }] : []),
  ]
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {layout === "single" && (
        <div className="space-y-6">
          {
            groups.length > 0 ? groups.map((group) => renderFieldsByGroup(group.name)) : renderFieldsByGroup() // Render all fields if no groups defined
          }
      ...
      {layout === "tabs" && allGroups.length > 0 && (
        <Tabs value={activeTab || allGroups[0].name} onValueChange={setActiveTab} className="w-full">
          <TabsList className={`grid w-full grid-cols-${Math.min(allGroups.length, 5)}`}>
            {allGroups.map((group) => (
          ...
          {allGroups.map((group) => (
            <TabsContent key={group.name} value={group.name} className="space-y-4 pt-4">
              {group.description && <p className="text-muted-foreground text-sm">{group.description}</p>}
              {renderFieldsByGroup(group.name === "__ungrouped__" ? undefined : group.name)}
            </TabsContent>
          ))}
        </Tabs>
      )}
