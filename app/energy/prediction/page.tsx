...
export default function CarbonPredictionPage() {
  ...
  const calculateAccuracy = () => {
    let totalDiff = 0;
    const count = Math.min(30, chartData.length); // 최근 30일 기준
    
    for (let i = 0; i < count; i++) {
      const diff = Math.abs(chartData[i].actual - chartData[i].predicted);
      totalDiff += diff / chartData[i].actual;
    }
  ...
  const predictionAccuracy = calculateAccuracy().toFixed(1);
